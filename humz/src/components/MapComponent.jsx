...
const AssaultComponent = ({ text }) => <div style={assaultStyles}>{text}</div>;
const TheftComponent = ({ text }) => <div style={theftStyles}>{text}</div>;
const CollisionComponent = ({ text }) => <div style={collisionStyles}>{text}</div>;
    ...
    render(){
        return (
            <div>
                ...
                defaultZoom={11}
                >
                {this.state.assaultData.map((incident)=> <AssaultComponent lat={incident.lat} lng={incident.long} text={incident.offence} />)}
                {this.state.theftData.map((incident)=> <TheftComponent lat={incident.lat} lng={incident.long} text={incident.offence} />)}
                {this.state.collisionData.map((incident)=> <CollisionComponent lat={incident.latitude} lng={incident.longitude} text={incident.acclass} />)}
